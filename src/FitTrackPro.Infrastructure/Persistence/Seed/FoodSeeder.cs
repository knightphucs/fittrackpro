...
public class FoodSeeder
{
    ...
    public FoodSeeder(ApplicationDbContext context, ILogger<FoodSeeder> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task SeedAsync()
    {
        try
        {
            _logger.LogInformation("Starting food database seeding...");

            var foods = SeedVietnameseFoodsAsync();
            int inserted = 0;

            foreach (var food in foods)
            {
                bool exists = await _context.Foods
                    .AnyAsync(e => e.Name == food.Name);
            ...
            _logger.LogInformation("Inserted {Count} new foods", inserted);
        }
        catch (Exception ex)
        {
